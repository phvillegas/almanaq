...
/**
 * The palette sheet, drawn from the same run.
 *
 * It used to be a hand-drawn SVG, and it went stale the moment the scheme became
 * generated: it kept showing hand-picked theme colours next to a brand that had moved
 * on. A reference that disagrees with the thing it references is worse than no
 * reference, so it is emitted here instead — from the same values, at the same moment.
 *
 * It paints itself in its own light scheme, which is a cheap sanity check: if the
 * generated neutrals were ever unreadable, this sheet would be the first thing to say so.
 */
function svg(): string {
  ...
  const COLUMNS = 6;
  const SWATCH = { width: 170, height: 110, gapX: 20, gapY: 96 };
  const MARGIN = 40;

  const parts: string[] = [];
  // Below the title, which sits on a baseline of 52. Starting at zero puts the first
  // heading straight through it.
  let y = 54;
  ...
  const heading = (text: string): void => {
    y += 56;
    parts.push(
      `  <text x="${MARGIN}" y="${y}" font-size="13" font-weight="600" letter-spacing="1.6" ` +
        `fill="${light.onSurfaceVariant}">${escape(text.toUpperCase())}</text>`,
    );
    y += 20;
  };

  const swatches = (entries: { name: string; hex: string; note: string }[]): void => {
    entries.forEach((entry, index) => {
      const column = index % COLUMNS;
      const row = Math.floor(index / COLUMNS);
      const x = MARGIN + column * (SWATCH.width + SWATCH.gapX);
      const top = y + row * (SWATCH.height + SWATCH.gapY);

      parts.push(
        `  <rect x="${x}" y="${top}" width="${SWATCH.width}" height="${SWATCH.height}" rx="14" ` +
          `fill="${entry.hex}" stroke="${light.outlineVariant}" />`,
        `  <text x="${x}" y="${top + SWATCH.height + 26}" font-size="15" font-weight="700" ` +
          `fill="${light.onSurface}">${escape(entry.name)}</text>`,
        `  <text x="${x}" y="${top + SWATCH.height + 48}" font-size="13" ` +
          `fill="${light.onSurfaceVariant}">${escape(entry.note ? `${entry.hex} · ${entry.note}` : entry.hex)}</text>`,
      );
    });

    y += Math.ceil(entries.length / COLUMNS) * (SWATCH.height + SWATCH.gapY);
  };

  // The role name is the label; repeating the word "role" underneath says nothing.
  const role = (name: string, scheme: Scheme) => ({ name, hex: scheme[name], note: '' });

  const SHOWN_ROLES = [
    'primary', 'onPrimary', 'primaryContainer', 'secondaryContainer', 'background', 'surface',
    'surfaceContainer', 'surfaceContainerHigh', 'onSurface', 'onSurfaceVariant', 'outlineVariant',
    'error',
  ];

  heading('Brand');
  swatches([
    { name: 'Vesper 500', hex: tokens.palette.vesper['500'], note: 'seed' },
    { name: 'Vesper 300', hex: tokens.palette.vesper['300'], note: 'brand, dark' },
    { name: 'Meridian 500', hex: tokens.palette.meridian['500'], note: 'accent' },
    { name: 'Meridian 400', hex: tokens.palette.meridian['400'], note: 'accent, dark' },
  ]);

  heading(`Material 3 · light · generated from ${SEED}`);
  swatches(SHOWN_ROLES.map((name) => role(name, light)));

  heading(`Material 3 · dark · generated from ${SEED}`);
  swatches(SHOWN_ROLES.map((name) => role(name, dark)));

  heading('Status · hand-picked, never generated');
  const statuses = Object.keys(tokens.status).filter((key) => !key.startsWith('$'));
  swatches(
    statuses.map((key) => ({
      name: key,
      hex: tokens.status[key].light.container ?? tokens.status[key].light.text,
      note: tokens.status[key].light.container ? 'container' : 'text, no container',
    })),
  );

  y += 56;
  const footer =
    'Generated by backend/scripts/build-theme.ts. The Material roles come from the seed; ' +
    'the status colours do not, and every status pair is checked at 4.5:1 before this file is written.';
  parts.push(
    `  <text x="${MARGIN}" y="${y}" font-size="13" fill="${light.onSurfaceVariant}">${escape(footer)}</text>`,
  );

  const width = MARGIN * 2 + COLUMNS * SWATCH.width + (COLUMNS - 1) * SWATCH.gapX;
...
<!--
  ...
  Written by backend/scripts/build-theme.ts, from design/tokens.json, in the same run
  that writes the Material roles and the Compose scheme. Run npm run build:theme.
-->
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"
     font-family="Inter, -apple-system, system-ui, sans-serif">
  <rect width="${width}" height="${height}" fill="${light.background}" />
  <text x="${MARGIN}" y="52" font-size="26" font-weight="700" fill="${light.onSurface}">Sol y Luna — palette</text>
${parts.join('\n')}
</svg>
`;
}

writeFileSync(join(here, '..', '..', 'design', 'palette.svg'), svg());
